---
sidebar_label: Athlete Profile
---

import ImageStepper from "../../src/components/ImageStepper";


# Athlete Profile

---

The Athlete Profile page is where you can manage and view all information and features about an individual athlete. To navigate to the profile of a particular athlete, select the athlete’s name from the Athlete Directory.

<ImageStepper>
  <ImageStepper.Step
    title="Athlete Profile"
    imgSrc="/img/docs/dashboard/Profile.png"
  >

  </ImageStepper.Step>
</ImageStepper>

## Managing Athlete

### Edit Athlete

If you would like to edit or update basic information about an athlete, the following can be changed here:
* First Name
* Last Name
* Birthday
* Sex
* Photo


<ImageStepper>
  <ImageStepper.Step
    title="Edit"
    imgSrc="/img/docs/dashboard/profile/edit-athlete.png"
    imgCaption="Make changes to basic athlete information"
  >

  </ImageStepper.Step>
</ImageStepper>

### Manage Positions

Each athlete can be assigned a primary and secondary position. 

#### Assign Position

To assign a new position, search for the position in the Position Search box.


<ImageStepper>
  <ImageStepper.Step
    title="Assign"
    imgSrc="/img/docs/dashboard/profile/edit-position.png"
    imgCaption="Search for the desired position(s) and assign it to an athlete"
  >

  </ImageStepper.Step>
</ImageStepper>

#### Remove Position

To remove a position, simply click the trash icon on the right of the list position. 
Don’t worry if you accidentally remove a position. The action can be undone, by simply clicking the reverse arrow.


<ImageStepper>
  <ImageStepper.Step
    title="Remove"
    imgSrc="/img/docs/dashboard/profile/remove-position.png"
    imgCaption="Remove unwanted position assignment"
  >

</ImageStepper.Step>
  <ImageStepper.Step
    title="Undo"
    imgSrc="/img/docs/dashboard/profile/undo-remove.png"
    imgCaption="Undo accidental position removal"
  >

  </ImageStepper.Step>
</ImageStepper>

#### Set Primary Position

The primary position of an athlete is what is used to determine nutrient requirements. To set the primary position, simply check the box to the left desired position below the athlete photo and confirm the selection.


<ImageStepper>
  <ImageStepper.Step
    title="Set Primary"
    imgSrc="/img/docs/dashboard/profile/set-primary.png"
    imgCaption="Choose from list of assigned positions to set primary position"
  >

</ImageStepper.Step>
  <ImageStepper.Step
    title="Verify Primary"
    imgSrc="/img/docs/dashboard/profile/verify-primary.png"
    imgCaption="View primary position on athlete profile"
  >

Once complete, you will see an orange check mark in the box. This can be changed at any time.

  </ImageStepper.Step>
</ImageStepper>



---

## Health Overview

The Health Overview section displays basic information about an athlete. You will find the athlete's age, birthday, sex, height and weight here. 

<ImageStepper>
  <ImageStepper.Step
    title="View"
    imgSrc="/img/docs/dashboard/profile/healthe.png"
    imgCaption="See basic health information on your athlete"
  >

  </ImageStepper.Step>
</ImageStepper>

To update this information see Edit Athlete.

To change the display units for height and weight see Account Settings > General.

---

## Allergies

The Allergies  section displays the allergies that the athlete reported in their Baseline Onboarding Questionnaire. 

These can only be updated by re-requesting the Baseline Questionnaire from the desired athlete. 

<ImageStepper>
  <ImageStepper.Step
    title="View"
    imgSrc="/img/docs/dashboard/profile/allergies.png"
    imgCaption="View allergy information about an athlete"
  >

  </ImageStepper.Step>
</ImageStepper>

---

## Risk Banners

### Active Injury

### Flag Banner

---

## Change Day View




